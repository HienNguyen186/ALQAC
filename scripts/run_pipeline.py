"""End-to-end ALQAC 2026 pipeline runner — SIMPLIFIED.

Chiến lược:
  - Tầng 1: BM25@1000 + Query Expansion → candidate pool
  - Tầng 2: Vietnamese Embedding rerank → top 20 articles
  - Tầng 3: Qwen3-8B direct-label (transformers.generate(), không vLLM API)

Mục tiêu: nhanh, simple, nhưng vẫn đạt accuracy cao nhờ retrieval tốt.
"""
...
def run_pipeline(
    ...
    llm_mode: str = "local",
    # ── Tầng 1: Multi-Retrieval (BM25 + Vietnamese) ─────────────
    bm25_top_k_articles: int = 1000,  # ✅ TĂNG lên 1000
    bm25_strategy: str = "hybrid",
    vn_model: str = "dangvantuan/vietnamese-embedding",
    vn_top_k: int = 200,
    fusion_top_k: int = 1000,
    # ── Tầng 2: Ensemble Reranker (Vietnamese Embedding only) ─
    final_min_k: int = 3,
    final_max_k: int = 20,   # ✅ Final output: 20 articles
    # ── Tầng 3: Prediction (Qwen3-8B) ──────────────────────────
    ...
    """Run simplified pipeline: BM25@1000 → VN rerank → Qwen3."""

    corpus_resolved = resolve_path(corpus_path, PROJECT_ROOT)

    # ── Tầng 1: Multi-Retriever (simplified) ────────────────────
    LOGGER.info("[Pipeline] Initializing Multi-Retriever (mode=%s, bm25_top_k=%d) ...",
                rerank_mode, bm25_top_k_articles)
    ...
    # ── Tầng 2: Ensemble Reranker (simplified) ──────────────────
    LOGGER.info("[Pipeline] Loading Ensemble Reranker (mode=%s, final_max_k=%d) ...",
                rerank_mode, final_max_k)
    ...
    LOGGER.info("[Pipeline] Loading Predictor (mode=%s, default_label=%s, direct-label) ...",
    ...
    for case in tqdm(cases, desc="Pipeline"):
        ...
        # Tầng 1: Multi-retrieval → BM25@1000 + VN@200 → RRF
        candidates = multi_retriever.retrieve(case_query, final_top_k=fusion_top_k)

        # Tầng 2: Vietnamese Embedding rerank → top 20
    ...
    out_path  = out_dir / f"submission_simplified_{timestamp}.json"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(description="ALQAC 2026 Simplified Pipeline")

    p.add_argument("--rerank-mode", default="local", choices=["mock", "local"])
    p.add_argument("--llm-mode",    default="local", choices=["mock", "local"])

    p.add_argument("--bm25-top-k-articles", type=int, default=int(rc.get("bm25_top_k_articles", 1000)))
    ...
    p.add_argument("--vn-top-k",  type=int, default=int(rc.get("vn_top_k", 200)))
    p.add_argument("--fusion-top-k", type=int, default=int(rc.get("fusion_top_k", 1000)))

    p.add_argument("--final-min-k", type=int, default=3)
    p.add_argument("--final-max-k", type=int, default=20)
